...
function buildLocalGalleryQuery() {
    return "
        SELECT 
            g.gallery_id,
            g.artist_id,
        ...
        WHERE g.gallery_id = ? AND g.gallery_type = 'physical' AND u.is_active = 1
...
function formatLocalGalleryData($row) {
    ...
    // Add gallery status and location info
    $gallery['status'] = [
        'is_active' => $gallery['is_active'],
        'is_currently_active' => $gallery['is_currently_active'],
        'status_text' => $gallery['is_currently_active'] ? 'Currently Open' : ($gallery['is_active'] ? 'Scheduled' : 'Inactive'),
        'gallery_type_text' => 'Physical Gallery',
        'access_type' => 'Physical Location - In-person visit required',
        'location_required' => true
    ];

    // Add location information
    $full_address = trim(($gallery['address'] ?: '') . ', ' . ($gallery['city'] ?: ''), ', ');
    $gallery['location_info'] = [
        ...
        'phone' => $gallery['phone'] ?: 'Phone not provided',
        'requires_visit' => true,
        'accessibility' => 'Physical gallery space',
        'visit_note' => 'Please visit the physical location during open hours'
    ];
...
function getLocalGalleryById($db, $gallery_id) {
    try {
        $query = buildLocalGalleryQuery();
        ...
        if (!$row) {
            throw new Exception("Local gallery not found", 404);
        }
        
        $stmt->close();
        
        return formatLocalGalleryData($row);
        
    } catch (Exception $e) {
        if ($e->getCode() === 404) {
            throw $e;
        }
        throw new Exception("Error fetching local gallery: " . $e->getMessage());
    }
}
...
function sendSuccessResponse($gallery) {
    $response = [
        'success' => true,
        'message' => 'Local gallery retrieved successfully',
        'data' => $gallery
    ];
...
function sendErrorResponse($message, $statusCode = 500) {
    error_log("getLocalGalleryInfo API Error: " . $message);
...
function handleGetLocalGalleryInfo() {
    global $db;
    ...
    try {
        ...
        // Get local gallery information
        $gallery = getLocalGalleryById($db, $gallery_id);
